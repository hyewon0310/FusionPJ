...
//    }
    ...
    public ClientHandler() {
        try {
            clientSocket = new Socket("172.30.77.23", 7777);
...
//    }
    ...
    private Protocol receive(int type, int code) throws Exception {
        System.out.println("[DEBUG] receive 함수 실행");
        int retryCount = 0;
        byte[] header = new byte[Protocol.LEN_HEADER];
        Protocol protocol = new Protocol();
        ...
        while (retryCount < MAX_RETRYCOUNT) {
            try {
                // 헤더 읽기
                System.out.println("[DEBUG] is에서 패킷 값 읽기 전");
                int bytesRead = is.read(header, 0, Protocol.LEN_HEADER);
                System.out.println("[DEBUG] 패킷에서 header 정보 읽음");
                if (bytesRead < Protocol.LEN_HEADER) {
    ...
    public String[] scheduleList_viewReq() throws Exception {
        ...
        String[] nameList= responseNameList.byteToStrArr(responseNameList.getBody(),40);
...
//        }
        ...
        for (int i = 0; i < nameList.length; i++)
        {
            result[i] = "입사 일정명 : " + nameList[i] + "\n입사 일정 기간 : " + periodList[i] +"\n";
            System.out.println();
        }
        return result;
    }
...
public int[] longin(String id, String pwd) throws Exception {
    ...
    public String getdormitoryID(String dormitoryName, String dormitoryType, String gender) throws Exception {
        ...
        viewRequest.setBody(dormitoryInfo, 10, 4);
        send(viewRequest);
        System.out.println("[DEBUG] receive 받기 전");
        Protocol viewResponse = receive(Protocol.TYPE7_VIEW_RES, Protocol.T7_CODE13_DORMITORY_ID);
        System.out.println("[DEBUG] receive 받은 후");
